(function(){
  ...
  var CONFIG = {
    ...
    /* Where form submissions are sent — your Google Apps Script web-app URL
       (see google-sheet-setup/SETUP.md). It ends in /exec.
       Leave '' and the form only shows a thank-you (nothing is delivered).
       Book Demo, Contact Us, Support and the newsletter box all send here;
       each row in the sheet says which one it came from. */
    formEndpoint: 'https://script.google.com/macros/s/AKfycbzcCQr61q_9MwUh2fgNjWaHQioJLQL8SyQwXjNR3yHYNQ6kQN268t-9Ayv1ZcQc2YDO/exec',

    /* WhatsApp business number, digits only, with country code. */
    whatsapp: '917757833382',
  ...
  /* remember which button opened the form, so the sheet shows
     "Book Demo" / "Contact Us" / "Support" for each lead */
  var formType = 'Book Demo';
  var demoTitle = document.getElementById('demoTitle');
  document.querySelectorAll('[data-demo]').forEach(function(b){ b.addEventListener('click', function(e){
    e.preventDefault();
    var t = b.textContent.trim();
    formType = /contact/i.test(t) ? 'Contact Us' : /support/i.test(t) ? 'Support' : 'Book Demo';
    demoTitle.textContent = formType === 'Book Demo' ? 'Book a demo' : formType;
    document.getElementById('demoForm').style.display = '';
    document.getElementById('demoDone').style.display = 'none';
    document.getElementById('demoErr').style.display = 'none';
    openModal('demoModal');
  }); });

  /* send one submission to the Google Sheet.
     text/plain keeps it a "simple" request, which Apps Script needs
     (it can't answer the CORS preflight that application/json triggers). */
  ...
  sendBtn.addEventListener('click', function(){
    ...
    sendLead(payload).then(function(){
      sendBtn.disabled = false; sendBtn.textContent = 'Request my demo';
      ['f-name','f-phone','f-outlet','f-city'].forEach(function(id){ document.getElementById(id).value = ''; });
      done();
    }).catch(function(err){
  ...
  /* newsletter — saved to the same sheet as type "Newsletter" */
  document.getElementById('newsBtn').addEventListener('click', function(){
    ...
    if(CONFIG.formEndpoint){
      sendLead({ type: 'Newsletter', email: email, source: 'siroo-landing', at: new Date().toISOString() })
        .catch(function(err){ console.error('SIROO: newsletter signup failed', err); });
    } else {
      console.warn('SIROO: CONFIG.formEndpoint is empty, so this signup was NOT saved.', email);
    }
